...
const gcrevoke = ["628571417094499-1621430788@g.us"]
blocked = []
...
const starts = async (Haikal = new WAConnection()) => {
    ...
    Haikal.on('qr', () => {
        console.log(color('[','white'), color('!','red'), color(']','white'), color(' Scan Bang By Hw Mods Wa'))
    })

    fs.existsSync('./session.json') && Haikal.loadAuthInfo('./session.json')
    Haikal.on('connecting', () => {
        start('2', 'Sabar Bot Xm Butuh Prosess...')
    })
    Haikal.on('open', () => {
        success('2', 'Bot Xm-Hw Sudah Aktif')
    })
